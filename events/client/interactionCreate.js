		...
		if (command) {
			if (command.permissions.length) {
				...
				for (const perm of command.permissions) {
					...
					if (!interaction.member.permissions.has(Discord.Permissions.FLAGS[perm])) invalidPerms.push(perm);
				}
				if (invalidPerms.length) return interaction.reply({ content: `Missing Permissions: \`${ invalidPerms + "".replace(/,/g, ", ") }\`` });
			}
			command.function(client, Discord, interaction, interaction.options);
